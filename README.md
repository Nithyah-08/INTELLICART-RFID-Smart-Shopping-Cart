# INTELLICART – RFID Smart Shopping Cart

## Overview
INTELLICART is an RFID-based smart shopping cart system designed to reduce checkout waiting time and improve shopping efficiency using embedded systems and RFID technology.

The project focuses on automating the billing process in supermarkets to provide a faster and smarter shopping experience.

## Features
- RFID-based product scanning
- Real-time LCD display updates
- Automated billing concept
- Real-time item detection
- Smart retail automation approach

## Components Used
- Arduino Nano
- EM-18 RFID Reader
- LCD Display
- Li-ion Battery
- Potentiometer
- Push Buttons
- Jumper Wires

## Software Used
- Arduino IDE
- Fusion 360

## Applications
- Smart retail systems
- Automated billing systems
- Inventory management
- Smart shopping solutions

## Published Paper
The project work was also documented in a published paper:

**“INTELLICART – An Advanced Method of Billing Systems Used in the Supermarket using Electronics”**

Published in:  
**Grenze International Journal of Engineering and Technology (2024)**
<img width="672" height="782" alt="Intellicart Paper SS" src="https://github.com/user-attachments/assets/df62c08c-b51b-4514-ab63-01a93a51772c" />

## Project Images
<img width="918" height="604" alt="Smart Shopping Cart" src="https://github.com/user-attachments/assets/56b4a253-db33-42b3-bda4-08b507f5433c" />
<img width="1028" height="403" alt="Reader Result 2" src="https://github.com/user-attachments/assets/af484b1b-1817-4d75-b96c-1ffe7b514dae" />
<img width="903" height="468" alt="Reader Result" src="https://github.com/user-attachments/assets/4e643aa2-fe31-472c-a475-bb81728dbe11" />

## Team
H NITHYA 
HARSHINI RAMANAN 
MAHESWAR REDDY O 
KRISHNA MURTHY N 
Mini Project – Department of Electronics & Communication Engineering  
Dayananda Sagar College of Engineering, Bengaluru
